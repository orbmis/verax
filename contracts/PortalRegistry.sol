...
contract PortalRegistry is Initializable {
  ...
  /**
   * @notice Get a Portal by its index
   * @param index The index of the portal within the portalAddresses array
   * @return The Portal
   */
  function getPortalByIndex(uint256 index) public view returns (Portal memory) {
    if (index >= portalAddresses.length) revert PortalNotRegistered();
    address id = portalAddresses[index];
    return portals[id];
  }
